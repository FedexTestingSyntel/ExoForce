...
public class WFCL{
	...
	@Test(dataProvider = "dp", enabled = false)
	public void WFCL_AccountRegistration_GFBO(String Level, String AdminUser, String Password, String Email){     //check on this, make sure that testing correct
		try {
			String ContactName[] = Helper_Functions.LoadDummyName("WADMInvite", Level);
			String Result[] = ContactName;//need to configure this
			//String Result[] = WFCL_Functions.WFCL_AccountRegistration_GFBO(Helper_Functions.LoadDummyName(AccountAddress[6]), Helper_Functions.LoadUserID(strLevel + "GFBO"), AccountNumber, AccountAddress);
			Helper_Functions.PrintOut(Arrays.toString(Result), false);
		}catch (Exception e) {
			Assert.fail(e.getMessage());
		}
